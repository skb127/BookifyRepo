...
internal sealed class ProcessOutboxMessagesJob : IJob
{
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        using IDbTransaction transaction = connection.BeginTransaction(); // Start a transaction to ensure atomicity

        IReadOnlyList<OutboxMessagesResponse> outboxMessages = await GetOutboxMessagesAsync(connection, transaction);

        foreach (OutboxMessagesResponse outboxMessage in outboxMessages)
        {
    ...
    private async Task<IReadOnlyList<OutboxMessagesResponse>> GetOutboxMessagesAsync(IDbConnection connection, 
        IDbTransaction transaction)
    {
        string sql = $"""
                      ...
                      FOR UPDATE
                      """; // FOR UPDATE locks the selected rows for processing

        IEnumerable<OutboxMessagesResponse> outboxMessages = await connection.QueryAsync<OutboxMessagesResponse>(sql, transaction: transaction);
    ...
    private async Task UpdateOutboxMessageAsync(IDbConnection connection,
        IDbTransaction transaction,
        OutboxMessagesResponse outboxMessage,
        Exception? exception)
    {
        const string sql = @"
