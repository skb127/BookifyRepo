-- Create Quartz schema if it does not exist
CREATE SCHEMA IF NOT EXISTS quartz;

-- Set search path to quartz schema for all subsequent statements
SET search_path TO quartz;

DO $$
  DECLARE DropDb INT := 0; -- Set to 0 to preserve tables, 1 to recreation
BEGIN
  IF DropDb = 1 THEN
...
-- Indexes (using CREATE INDEX IF NOT EXISTS to avoid errors on duplicate execution)
