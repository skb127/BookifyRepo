...
public class SearchRateLimitTests : RateLimitIntegrationTest
{
    ...
    public async Task Search_ShouldReturnTooManyRequests_WhenLimitExceeded_ForAuthenticatedUser()
    {
        ...
        string accessToken = await GetAccessTokenAsync(RateLimitUserData.SearchUser.Email, RateLimitUserData.SearchUser.Password);
        HttpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", accessToken);

        // Send 2 requests (should be OK)
        for (int i = 0; i < 2; i++)
        {
            var response = await HttpClient.GetAsync("api/v1/apartments?startDate=2024-01-01&endDate=2024-01-10");
            response.StatusCode.Should().NotBe(HttpStatusCode.TooManyRequests);
        }

        // 3rd request should fail
        var exceededResponse = await HttpClient.GetAsync("api/v1/apartments?startDate=2024-01-01&endDate=2024-01-10");
