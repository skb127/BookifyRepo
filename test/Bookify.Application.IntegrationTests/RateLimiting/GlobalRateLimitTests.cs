...
public class GlobalRateLimitTests : RateLimitIntegrationTest
{
    ...
    public async Task GlobalLimit_ShouldReturnTooManyRequests_WhenLimitExceeded_ForAuthenticatedUser()
    {
        ...
        string accessToken = await GetAccessTokenAsync(RateLimitUserData.GlobalLimiterUserA.Email,
            RateLimitUserData.GlobalLimiterUserA.Password);
        HttpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", accessToken);

        // Send 3 requests (should be OK) - targeting a GET endpoint without specific policy
        for (int i = 0; i < 3; i++)
        {
            var response = await HttpClient.GetAsync(new Uri("api/v1/users/me", UriKind.Relative));
            response.StatusCode.Should().NotBe(HttpStatusCode.TooManyRequests);
        }

        // 4th request should fail
        var exceededResponse = await HttpClient.GetAsync(new Uri("api/v1/users/me", UriKind.Relative));
        ...
        string accessTokenB = await GetAccessTokenAsync(RateLimitUserData.GlobalLimiterUserB.Email,
            RateLimitUserData.GlobalLimiterUserB.Password);
        HttpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", accessTokenB);

        var responseUserB = await HttpClient.GetAsync(new Uri("api/v1/users/me", UriKind.Relative));
